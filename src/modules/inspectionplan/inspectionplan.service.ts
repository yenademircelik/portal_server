...
export class InspectionPlanService {
  constructor(
    @Inject(INSPECTIONPLAN_REPOSITORY)
    private readonly inspectionPlanRepository: typeof InspectionPlan,
  ) {}

  async createInspectionPlan(
    inspectionPlan: InspectionPlanDto,
  ): Promise<InspectionPlan> {
    return await this.inspectionPlanRepository.create(inspectionPlan);
  }
  async getByStateAndStatus(
    state: string,
    status: string,
  ): Promise<InspectionPlan[]> {
    return await this.inspectionPlanRepository.findAll<InspectionPlan>({
      where: Sequelize.and({ state }, { status }),
    });
  }
  ...
  async delete(vendor_odooid: number): Promise<InspectionPlan> {
    const deleteInspectionPlan = await this.inspectionPlanRepository.destroy({
      where: { vendor_odooid },
    });
  ...
  async update(
    ...
    const updateResult = await this.inspectionPlanRepository.update(
      fieldsToUpdate,
      {
        where: { vendor_odooid },
      },
