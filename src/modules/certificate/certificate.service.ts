...
export class CertificateService{
    constructor(
        @Inject(CERTIFICATE_REPOSITORY) private readonly certificateRepository:typeof Certificate
    ){}
    ...
    async update(id: number, fieldsToUpdate: Record<string, any>): Promise<Certificate[] | null> {
        const updateResult = await this.certificateRepository.update(fieldsToUpdate, {
            where: { id }
        });
     ...
     async delete(id:number):Promise<Certificate>{
        const deleteCertificate= await this.certificateRepository.destroy({where:{id}})
