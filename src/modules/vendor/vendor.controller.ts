...
export class VendorController{
    ...
    async createVendors(@Body() vendor:VendorDto){
        ...
        try {
            ...
            throw new HttpException('Bad Request at Creating Vendor',HttpStatus.BAD_REQUEST)
    ...
    @UseGuards(JwtGuard)
    @Get('search/:name')
    async getVendorsByName(@Param('name') name:string ){
        const vendor=await this.vendorService.getVendorsByName(name)
        try {
            return (vendor)

        }catch(error){
            throw new HttpException('Bad Request at Finding Vendor',HttpStatus.BAD_REQUEST)
